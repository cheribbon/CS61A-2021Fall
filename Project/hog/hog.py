...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        compared_score = 0
        if who == 0:
            compared_score = score0
        else:
            compared_score = score1
        new_high = abs(compared_score - last_score)
        if compared_score > last_score and new_high > running_high:
            print("{0} point(s)! That's a record gain for Player {1}!".format(new_high, who))
            return announce_highest(who, compared_score, new_high)
        else:
            return announce_highest(who, compared_score, running_high)
    return say
    # END PROBLEM 7
