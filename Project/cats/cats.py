...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(paragraph):
        text = split(lower(remove_punctuation(paragraph)))
        return any([x in text for x in topic])
    return func
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    closest_word = min(valid_words, key = lambda word: diff_function(typed_word, word, limit))
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        num = min(player_indices, key = lambda player_num: time(match, player_num, word_index))
        lst[num] += [word_at(match, word_index)]
    return lst
    # END PROBLEM 10
